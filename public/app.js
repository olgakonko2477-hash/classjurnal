...
function render() {
  ...
  if(page==='settings'&&session.cloud) $('.settings-grid').insertAdjacentHTML('beforeend','<section class="card settings-card"><h2>Копии перед восстановлением</h2><p>Последние 10 состояний журнала перед заменой данных из файла.</p>'+btn('Открыть сохранённые копии','snapshots',null,'secondary')+'</section>');
}
function onboarding() {
...
document.addEventListener('click',async e=>{
  ...
  try{
    switch(act){
      ...
      case 'snapshots':{const list=await api('/api/snapshots');modal('Копии перед восстановлением',list.length?`<div class="day-list">${list.map(x=>`<a class="day-list-lesson" href="/api/snapshot?id=${encodeURIComponent(x.id)}" download><span>${esc(new Date(x.created_at).toLocaleString('ru'))}</span>${icon('download')}</a>`).join('')}</div>`:'<p class="quiet">Восстановление из файла ещё не выполнялось.</p>',async()=>{});break;}
